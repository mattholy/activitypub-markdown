        ...
        if (customRenderers) {
            switch (node.type) {
                ...
                case 'text':
                    ...
                    return h('span', { 'data-node-type': node.type, 'data-node-style': 'default' }, { default: () => (node.value) });
                case 'paragraph':
                    if (customRenderers?.paragraph) {
                        return customRenderers.paragraph(node);
                    }
                    return h('p', { 'data-node-type': node.type, 'data-node-style': 'default' }, { default: () => (node.children || []).flatMap((i) => renderNode(i, customRenderers)) });
                case 'heading':
                    if (customRenderers?.heading) {
                        return customRenderers.heading(node);
                    }
                    return h(`h${node.depth}`, { 'data-node-type': node.type, 'data-node-style': 'default' }, { default: () => (node.children || []).flatMap((i) => renderNode(i, customRenderers)) });
                case 'emphasis':
                    if (customRenderers?.emphasis) {
                        return customRenderers.emphasis(node);
                    }
                    return h('em', { 'data-node-type': node.type, 'data-node-style': 'default' }, { default: () => (node.children || []).flatMap((i) => renderNode(i, customRenderers)) });
                case 'strong':
                    if (customRenderers?.strong) {
                        return customRenderers.strong(node);
                    }
                    return h('strong', { 'data-node-type': node.type, 'data-node-style': 'default' }, { default: () => (node.children || []).flatMap((i) => renderNode(i, customRenderers)) });
                case 'inlineCode':
                    if (customRenderers?.inlineCode) {
                        return customRenderers.inlineCode(node);
                    }
                    return h('code', { 'data-node-type': node.type, 'data-node-style': 'default' }, { default: () => node.value });
                case 'code':
                    if (customRenderers?.code) {
                        return customRenderers.code(node);
                    }
                    return h('pre', { 'data-node-type': node.type, 'data-node-style': 'default' }, { default: () => h('code', { 'data-node-style': 'default' }, node.value) });
                case 'blockquote':
                    if (customRenderers?.blockquote) {
                        return customRenderers.blockquote(node);
                    }
                    return h('blockquote', { 'data-node-type': node.type, 'data-node-style': 'default' }, { default: () => (node.children || []).flatMap((i) => renderNode(i, customRenderers)) });
                case 'list':
                    if (customRenderers?.list) {
                        return customRenderers.list(node);
                    }
                    return h(node.ordered ? 'ol' : 'ul', { 'data-node-type': node.type, 'data-node-style': 'default' }, { default: () => (node.children || []).flatMap((i) => renderNode(i, customRenderers)) });
                case 'listItem':
                    if (customRenderers?.listItem) {
                        return customRenderers.listItem(node);
                    }
                    return h('li', { 'data-node-type': node.type, 'data-node-style': 'default' }, { default: () => (node.children || []).flatMap((i) => renderNode(i, customRenderers)) });
                case 'thematicBreak':
                ...
                case 'table':
                    ...
                    return h('table', { 'data-node-type': node.type, 'data-node-style': 'default' }, { default: () => (node.children || []).flatMap((i) => renderNode(i, customRenderers)) });
                case 'tableRow':
                    if (customRenderers?.tableRow) {
                        return customRenderers.tableRow(node);
                    }
                    return h('tr', { 'data-node-type': node.type, 'data-node-style': 'default' }, { default: () => (node.children || []).flatMap((i) => renderNode(i, customRenderers)) });
                case 'tableCell':
                    if (customRenderers?.tableCell) {
                        return customRenderers.tableCell(node);
                    }
                    return h('td', { 'data-node-type': node.type, 'data-node-style': 'default' }, { default: () => (node.children || []).flatMap((i) => renderNode(i, customRenderers)) });
                case 'delete':
                    if (customRenderers?.delete) {
                        return customRenderers.delete(node);
                    }
                    return h('del', { 'data-node-type': node.type, 'data-node-style': 'default' }, { default: () => (node.children || []).flatMap((i) => renderNode(i, customRenderers)) });
                case 'html':
